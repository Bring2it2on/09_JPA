...
public class EmbeddedKeyTests {
    ...
    * 복합키가 존재하는 테이블의 매핑
    * 1. Embedded
    *       - @Embeddable 클래스에 복합키를 정의하고 엔티티에 @EmbeddedId를 이용하여 매핑
    ...
    *       - 코드가 간결하다.
    *
    * @EmbeddedId가 조금 더 객체지향적인 방식이고, IdClass가 관계형 데이터 베이스에 가까운 방식.
    * 두 방식 모두 복합키 클래스 영속성 컨텍스트가 관리하지 않는다는 특징을 가진다.
    *
    * 주의할 점
    * 1. 복합키의 매핑에서 복합키 클래스에 equals, hashCode 매서드를 구현해야한다.
    * -> JPA 엔티티 객체의 동일성을 판단하기 위해 필요
    *
    * 2. @GeneratedValue 어노테이션을 사용해서 복합키를 자동으로 생성하는 것은 불가능
    *
    * */
